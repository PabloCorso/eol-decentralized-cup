...
In this example, the `Lvl 3` is the top ranked level even though it only has 1 finish (1 PR). The `Lvl ` is last at 3rd position, even though is the level with most finishes and with a spam finish of over an hour.
